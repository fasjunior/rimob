angular.module("Rimob")
    .controller("loginCtrl", ["$scope", "$rootScope", "$location", "$mdDialog", "$timeout", "$dbOff", "parametroService",
                               "exportacaoService", "notificacaoService", "loginService", "authService", "appConfig",
                     function ($scope, $rootScope, $location, $mdDialog, $timeout, $dbOff, parametroService,
                                exportacaoService, notificacaoService, loginService, authService, appConfig) {
        ...
        $scope.obterAgenteImobiliario = function (agente) {
            ...
            .then(function (response) {
                ...
                return loginService.obterAgenteImobiliario(null, agente.NuCpf, agente.DsSenha);
            })
        ...
        $scope.logoutAgente = function () {
            let confirm = $mdDialog.confirm()
              ...
              .textContent('O Rimob removerá todos os dados e depois será reinicializado.')
              .ariaLabel('Logout')
              .ok('Sim')
              .cancel('Não');
        ...
        $scope.controlSideBarMenu = function (option) {
            ...
            if (!route.includes("/imovel/bairros/logradouros/imoveis/") && (route != '/login')) {
                $('#body').removeClass();
                if (option == 0) {
                    $('#body').addClass('skin-purple-light sidebar-mini ng-scope');
                }
                else {
                    $('#body').addClass('skin-purple-light sidebar-mini ng-scope sidebar-open');
                }
            }
        ...
        this.adicionaMensagemInicial = function () {
            ...
            let msg = "Bem-vindo ao Rimob! Antes de começar a utilizar o Rimob é necessário importar os dados. A opção está disponível na tela principal do Rimob."
        ...
        this.login = function (agente) {
            authService.obterSituacaoEmpresa(agente.NuCnpj)
            .then(function (response) {
                let situacaoEmpresa = angular.fromJson(response.data[0]);
                let msg = '';

                if (situacaoEmpresa.FlSistemabloqueado) {
                    msg = 'Unidade Gestora inválida. Não é possível entrar no Rimob. Contate o suporte.';
                }
