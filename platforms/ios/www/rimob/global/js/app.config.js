...
rimobApp.config(['$qProvider', '$httpProvider', '$mdDateLocaleProvider', 'panelsProvider', function ($qProvider, $httpProvider, $mdDateLocaleProvider, panelsProvider) {
...
rimobApp.constant('appConfig',
{
    ...
    credential: { user: 'Rimob', password: 'XLkVnion5n73xWW2MSIR0WbHePnAIfCSGKBCJVXIDW0=', sqSistema: 25 },
    urlService: {
        tributos:
        {
            bairro: 'bairro',
            ...
            entrevistadorecadastramento_max_codigo: 'entrevistadorecadastramento/max-codigo-por-agente/',
            imovelrecadastramento_max_codigo: 'imovelrecadastramento/max-codigo-por-agente/',
            contribuinterecadastramento_max_codigo: 'contribuinterecadastramento/max-codigo-por-agente/'
...
rimobApp.run(["$rootScope", "$dbOff", "$location", "$q", "$mdSidenav", "parametroService", "exportacaoService", "notificacaoService", "importacaoService", "errorInterceptorService", "appConfig", function ($rootScope, $dbOff, $location, $q, $mdSidenav, parametroService, exportacaoService, notificacaoService, importacaoService, errorInterceptorService, appConfig) {
    ...
    $rootScope.$on('$routeChangeStart', function (evt, next, current) {
        ...
        if (route == '/login'){
            ...
            .then(function () {
                if ((route != '/login/unidadegestora') && ($rootScope.parametroAgente != null))
                ...
                result.filter(function (notificacao) {
                    if (notificacao.TpNotificacao == 3) {
                        ...
                        if (notificacao.FlLida == 0) {
                            $rootScope.qtMensagensNaoLidas = $rootScope.qtMensagensNaoLidas + 1;
                        }
                    }
                    if (notificacao.TpNotificacao == 1 || notificacao.TpNotificacao == 2) {
    ...
    this.checkExportacao = function () {
        let deferred = $q.defer();
        exportacaoService.obterExportacoesNaoRealizadas()
          .then(function (tabelas) {
              //$rootScope.tabelasNaoExportadas = tabelas;
              if (tabelas.length > 0) {
                  notificacaoService.obterNotificacao(1, 2).then(function (notificacoes) {
                      if (notificacoes == undefined || notificacoes.length == 0) {
                          ...
                          let notificacao = { TpNotificacao: 1, SqNotificacao: 2, DsTitulo: 'Exportação', DsConteudo: msg, DtNotificacao: data, FlLida: 0 };
    ...
    this.checkImportacao = function () {
        let deferred = $q.defer();
        importacaoService.obterImportacoesNaoRealizadas()
          .then(function (tabelas) {
              $rootScope.tabelasNaoImportadas = tabelas;
              if (tabelas.length > 0) {
                  notificacaoService.obterNotificacao(1, 1).then(function (notificacoes) {
                      if (notificacoes == undefined || notificacoes.length == 0) {
                          ...
                          msg = tabelas.length + msg;
                          let data = new Date().toLocaleDateString("pt-BR");
                          let notificacao = { TpNotificacao: 1, SqNotificacao: 1, DsTitulo: 'Importação', DsConteudo: msg, DtNotificacao: data, FlLida: 0 };
                          notificacaoService.inserirNotificacao(notificacao);
                      }
                      deferred.resolve();
                  });
    ...
    this.checkParametro = function (nextRoute) {
        ...
        if ($rootScope.unidadesGestora) {
            ...
            parametroService.obterParametro().then(function (result) {
                ...
                return $dbOff.executeQuery('select SqAgenteimobiliario, NmAgenteimobiliario, NuCpf, NuCnpj, NmUnidgestora, CdMunicipio, DsFormulanumpublico, SqImovelrecadastramentomax, SgUf from Parametro;');
            })
