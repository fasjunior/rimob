angular.module("Rimob").controller("showBackEndImportCtrl", [
  "$scope",
  "$rootScope",
  "$mdDialog",
  "$location",
  ...
  function (
    ...
    imovelService.obterTodosImoveis().then(function (result) {
      if (result && result.length > 0) {
        ...
        $rootScope.imoveisFinalizados = $scope.imoveis.filter(function (
          imovel
        ) {
        ...
        $scope.progressImoveis = Math.round(
          ($rootScope.imoveisFinalizados.length / $scope.totalImoveis) * 100
        );
      } else {
        $scope.totalImoveis = 0;
        $scope.progressImoveis = 0;
    ...
    contribuinteService.obterQtContribuintes().then(function (result) {
      $scope.totalContribuintes =
        result && result[0] ? result[0].qtContribuintes : 0;
    });

    contribuinteService.obterQtNovosContribuintes().then(function (result) {
      $rootScope.novosContribuintes =
        result && result[0] ? result[0].qtContribuintes : 0;
    });
    ...
    $scope.showBackEndImport = function () {
      if ($rootScope.parametroAgente && !$rootScope.unidadesGestora) {
        ...
        if (
          $rootScope.imoveisFinalizadosNaoExportados.length > 0 ||
          $rootScope.novosContribuintes > 0
        ) {
          msg =
            "ATENÇÃO! Foram encontradas alterações feitas pelo agente, estas alterações serão perdidas após a importação, caso queira armazená-las envie os dados antes da importação.";
        }
