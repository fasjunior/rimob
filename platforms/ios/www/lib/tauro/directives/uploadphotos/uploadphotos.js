(function () {
  ...
  tauro.directive("taUploadPhotos", [
    "$mdDialog",
    "$q",
    function ($mdDialog, $q) {
      return {
        restrict: "E",
        templateUrl:
          "lib/tauro/directives/uploadphotos/templates/uploadphotos.html",
        scope: {
          images: "=",
        ...
        controller: function ($scope, $element, $attrs) {
          ...
          $scope.resize = function (img, maxW, maxH) {
            ...
            if (widthIsSmaller) {
              ...
              img.height = Math.round(
                img.height - Math.round((img.height * ratio) / 100)
              );
              img.width = maxW;
            } else {
              let ratio = 100 - Math.round((maxH * 100) / img.height);
              img.width = Math.round(
                img.width - Math.round((img.width * ratio) / 100)
              );
              img.height = maxH;
            }
          };
          ...
          $scope.showPicture = function (selectedImage) {
            $mdDialog.show({
              ...
              templateUrl:
                "lib/tauro/directives/uploadphotos/templates/showphoto.html",
            ...
            if (
              ...
              $mdDialog.show(
                $mdDialog
                  ...
                  .title("Atenção")
                  .textContent(
                    "O limite de fotos por exportação foi excedido! Faça a exportação dos dados para liberar espaço e poder adicionar mais fotos."
                  )
              ...
              navigator.camera.getPicture(
                ...
                function (error) {
                  if (
                    !error.includes("has no access to assets") &&
                    !error.includes("no image selected") &&
                    !error.includes("Camera cancelled") &&
                    !error.includes("Selection cancelled")
                  ) {
                    $mdDialog.show(
                      $mdDialog
                        .alert()
          ...
          function DialogController($scope, $mdDialog, image) {
            ...
            $scope.selectedImage.srcComplete =
              "data:image/jpeg;base64," + $scope.selectedImage.Imagem;
            $scope.hide = function () {
              $mdDialog.hide();
            };
