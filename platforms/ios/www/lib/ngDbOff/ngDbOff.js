...
(function () {
    ...
    angular.module('ngDbOff').service("$dbOff", ["$http", "$q", "$rootScope", "$mdDialog", "appConfig", function ($http, $q, $rootScope, $mdDialog, appConfig) {
        /*
        ...
        */
        ...
        this.getDataBaseModel = function () {
            let o = this;
            return $http.get("lib/ngDbOff/dboff.database.json").then(function (response) {
        ...
        this.openDB = function () {
            ...
            if (databaseModel) {
                db = window.openDatabase(databaseModel.dataBase, "1.0", databaseModel.dataBase, 2 * 1024 * 1024);
                if (!db) {
        ...
        */
                ...
                if (db) {
                    db.transaction(function (tx) {
                        for(let table of databaseModel.tables)
                        {
                            ...
                            tx.executeSql(sqlTable, [], function (tx, result) {
        ...
        this.checkDB = function () {
            ...
            .then(function (response) {
                dbID           = (response && response[0] && response[0].id) ? response[0].id : 0;
            ...
            .then(function (response) {
                scripts = (response && response.data) ? angular.fromJson(response.data) : [];
                if ((requiredUpdate) && (angular.element(document).find('md-dialog').length <= 0)) {
                    $mdDialog.show({
        ...
        this.executeQuery = function (query) {
            ...
            db.transaction(function (tx) {
                for (let sqlTransaction of queries) {
                    let deferred = $q.defer();
                    tx.executeSql(sqlTransaction, [], function (tx, result) {
                        let data = [];
                        ...
                        console.log('executeQuery: ' + sqlTransaction + '. Error: ' + error.message);
                        deferred.resolve(null);
                    });
                    promises.push(deferred.promise);
                }
        ...
        this.tableExists = function (tableName) {
            for(let table of databaseModel.tables)
            {
                if (table.tableName.toLocaleLowerCase() == tableName.toLocaleLowerCase()) {
                    return true;
                }
            }
            return false;
        ...
        this.insert = function (tableName, jsonData) {
            ...
            angular.forEach(jsonData, function (item) {
                ...
                Object.keys(jsonData[0]).forEach(function (key) {
                    ...
                    if (Object.keys(table.columns).filter(function (v) { return v.toLowerCase() === columnName.toLowerCase(); }).length > 0) {
                        sqlQuery += columnName + ",";
                        parameters += "?,";
            ...
            db.transaction(function (tx) {
                for (let sqlTransaction of sqlTransactions) {
                    let deferred = $q.defer();
                    tx.executeSql(sqlTransaction.sqlQuery, sqlTransaction.sqlParameters, function (tx, result) {
                        deferred.resolve(true);
                    }, function (tx, error) {
                        console.log(error.message);
                        deferred.resolve(null);
        ...
        */
        this.selectAll = function (tableName, options) {
            ...
            if (options && options.max) {
                ...
                angular.forEach(table.columns, function (value, key) {
                    let columnName = key;
                    if (options && (options.join || options.leftjoin) && options.columnsjoin)
            ...
            if (options && (options.leftjoin || options.join) && options.columnsjoin) {
                let optionJoin = options.leftjoin ? "left join " : "join ";
                sqlQuery += optionJoin + options.leftjoin + " on ";
                if (options.columnsjoin) {
                    sqlQuery += o.setColumnsJoin(tableName, options.leftjoin, options.columnsjoin) + " ";
                }
                if (options.onjoin) {
                    sqlQuery += options.columnsjoin ? "and " + options.onjoin + " " : options.onjoin + " ";
                }
            }

            if (options && options.where) {
                sqlQuery += "where " + o.replaceParametersWithValues(options.where, options.parameters) + " ";
            }
        ...
        this.update = function (tableName, options) {
            ...
            if (options && options.set) {
                sqlQuery += o.replaceParametersWithValues(options.set, options.parametersSet) + " ";
            }
            else {
            ...
            let deferred = $q.defer();
            db.transaction(function (tx) {
                tx.executeSql(sqlQuery, sqlParameters, function (tx, result) {
                    deferred.resolve(result);
                }, o.onError);
            });
        ...
        this.isPrimaryKey = function (table, columnName) {
            ...
            if (table.primaryKey) {
                ...
                for (let i = 0; i < primaryKeyArray.length; i++) {
                    if (primaryKeyArray[i].trim().toLowerCase() == columnName.toLowerCase()) {
                        return true;
                    }
                }
            }
        ...
        function dialogUpdateDB($scope, $mdDialog, $route, $timeout) {
            $scope.status = "Atualizando a estrutura do Rimob para a nova versão...";
            $scope.completed = 0;
            $scope.hide = function () {
                $mdDialog.hide();
            };
            ...
            $scope.$on('updateDB', function (event, args) {
                $timeout(function () {
                    $scope.status = (args.completed == 1) ? "Atualização concluida com sucesso!" : "Falha ao tentar atualizar!";
